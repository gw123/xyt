...
<div class="container">
    <div class="row" style="margin-bottom: 15px;margin-top: 10px;">
        <div class="col-xs-6 col-sm-8">
            <h3 class=" " style="color: #c88d27;">书籍列表</h3>
        </div>
        <div class="col-xs-6 col-sm-4" style="text-align: right">
            <?=Html::beginForm('/index/book','get')?>
    ...
    <div class="imageDisplay clearfix">
        <div id="myCarousel" class="carousel slide">
            <?php $carsousels = include_once (Yii::getAlias('@frontend/config/carousel.php')); ?>
            <!-- 轮播（Carousel）指标 -->
            <ol class="carousel-indicators">
                <?php foreach ($carsousels as $index =>$v) { ?>
                     <?php if ($index==0) {?>
            ...
            <div class="carousel-inner">
                <?php foreach ($carsousels as $index =>$v) { ?>
                <?php if ($index==0) {?>
                    <div class="item active"> <img src="<?=$v?>" alt="First slide"></div>
                <?php }else{ ?>
                   <div class="item "> <img src="<?=$v?>" alt="First slide"></div>
                    <?php } ?>
                <?php } ?>
            </div>
        </div>
    ...
    <ul id="lesson_list" class="row-list-box2 clearfix">
            ...
            foreach ($lists as $item) {?>
                <li class=" col-sm-4 col-xs-6 col-md-3">
                    <div style="width: 240px;position: relative">
                        <div class="li-box">
                            <h5 class="item-name" title="<?=$item['title']?>">
                                <a href="<?='/index/book-detail?bookId='.$item['id']  ?>"> <?=$item['title']?></a>
                            </h5>
                            <div class="item-about" title="<?=$item['desc']?>">
                                <a href="<?='/index/book-detail?bookId='.$item['id']  ?>">  <?=$item['desc']?> </a>
                            </div>
...
<script type="text/javascript">
    ...
    $('.row-list-body li').mouseenter(function(e) {
        ...
        $(this).find('img').animate({ left: '0', top: '0', width:'100%',height:'240px'}, 300);
    });
